...
var Tip = Class.create({
  initialize: function(element, content) {
    ...
    if (opt.style) {
    	opt = Object.extend(Object.clone(Prototip.Styles[opt.style]), opt);
    }
...
function __prototip_actions_init(actionFunc) {
	actionFunc.createTip = function(selector, ajaxrequest, contentHtml, 
		...
		$Elements(selector).each(function(element) {
			...
			ajaxrequest = ajaxrequest || actionFunc.ajaxrequest;
			if (ajaxrequest) {
				c.update($MessageConst["$Loading.0"]);			
				element.observe("prototip:shown", function() {
					var ar = Object.isString(ajaxrequest) ? 
							$Actions[ajaxrequest] : ajaxrequest;
					ar.container = c;
				...
				actionFunc.ajaxrequest = ajaxrequest;
			} else {
				if (contentHtml) {
