...
UI.WindowManager = Class.create(UI.Options, {
	...
	initialize : function(options) {
		...
		this.container.observe('drag:started', this.onStartDrag.bind(this))
				.observe('drag:updated', this.onDrag.bind(this)).observe(
						'drag:ended', this.onEndDrag.bind(this));
	...
	removeOverflow : function() {
		...
		if (this.container == document.body)
			this.cssRule = $UI.addStyleSheet("html { overflow: hidden; }");
	},
...
function __window_actions_init(actionFunc, name, single) {
	...
	actionFunc.createWindow = function(url, contentStyle, contentHtml, contentRef) {
		var pWin = $Actions[name];
		if (pWin && single && pWin.isVisible())
			return;
		...
		if (url) {
			...
			if (contentStyle) {
				_content.setStyle(contentStyle);
			}
			if (contentHtml) {
				_content.update(contentHtml);
			} 
			win.setContent(_content);
	...
	actionFunc.showWindow = function(title, popup, center, modal, destroyOnClose, params) {
		...
		var t = opt.title || title;
		if (t)
			win.setHeader(t);
		
		if (destroyOnClose || !_content.loaded) {
		...
		if (popup) {
			...
			win.setPosition(p[1], p[0]).show().activate();
		} else {
			if (center) {
				win.center();
			}
