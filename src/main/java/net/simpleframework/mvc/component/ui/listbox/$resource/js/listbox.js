...
$UI.ListBox = Class.create( {
	initialize : function(container, datas, options) {
		...
		Event.observe(document, 'keydown', this._keyPress.bindAsEventListener(this));
	...
	_upDownAction : function(up) {
		if (this.current != null) {
			var sibling = up ? this.current.element.previousSibling : this.current.element.nextSibling;
